	...
	if not Bundle then
		...
		if not Success then
			...
			if not EspErrorReported then
				...
				warn("Player ESP drawing creation error: " .. tostring(Result))
			end
			return nil
		end
...
TrackConnection(RunService.RenderStepped:Connect(function()
	...
	if not Flags.Running or not Flags.EspEnabled or EspRendererFailed then
		return
	end
	...
	for _, Player in Players:GetPlayers() do
		local Success, ErrorMessage = pcall(function()
			...
			if Target then
				...
				if Bundle then
					UpdateEspBundle(Bundle, Target, Camera)
				end
			end
		end)
		if not Success and not EspErrorReported then
			EspErrorReported = true
			warn("Player ESP renderer error: " .. tostring(ErrorMessage))
		end
	end
end))
